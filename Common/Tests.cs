...
namespace Common
{
    ...
    public class Tests
    {
        public static bool TestUnDirectedGraphs(Action<string> onProgress, Action<string> onError)
        {
            ...
            onProgress($"Test graph:\n{graph.ToString()}");

            onProgress("Testing AddVertex(), AddEdge(), VertexCount() and EdgeCount()...");

            int numVertices = graph.VertexCount();
            if (numVertices != 4)
            {
                onError($"Error. VertexCount() returned {numVertices} instead of 4");
                return false;
            }
            ...
            if (numEdges != 6)
            {
                onError($"Error. EdgeCount() returned {numEdges} instead of 6");
                return false;
            }
            ...
            if (numEdges != 6)
            {
                onError($"Error. AddEdge() with existing edge failed. Added a new edge instead of updating the value");
                return false;
            }
            onProgress("Ok.");

            onProgress("Testing GetEdgeWeight() with existing edges...");
            if (graph.GetEdgeWeight("Maider", "Itxaso") != 3.12)
            {
                onError($"Error. GetEdgeWeight(\"Maider\",\"Itxaso\") returned {graph.GetEdgeWeight("Maider", "Itxaso")} instead of 3.12");
                return false;
            }

            if (graph.GetEdgeWeight("Alaitz", "Galder") != 16.2)
            {
                onError($"Error. GetEdgeWeight(\"Alaitz\",\"Galder\") returned {graph.GetEdgeWeight("Alaitz", "Galder")} instead of 16.2");
                return false;
            }
            onProgress("Ok.");

            onProgress("Testing GetEdgeWeight() with non-existing edge...");
            if (graph.GetEdgeWeight("Maider", "Pedro") != 0)
            {
                onError($"Error. GetEdgeWeight(\"Maider\",\"Pedro\") returned {graph.GetEdgeWeight("Maider", "Pedro")} instead of 0");
                return false;
            }
            onProgress("Ok.");

            onProgress("Testing RemoveEdge() with existing edge...");
            graph.RemoveEdge("Alaitz", "Galder");
            numEdges = graph.EdgeCount();
            if (numEdges != 5)
            {
                onError($"Error. EdgeCount() returned {numEdges} instead of 5");
                return false;
            }
            onProgress("Ok.");

            onProgress("Testing RemoveEdge() with non-existing edge...");
            graph.RemoveEdge("Alaitz", "Galder");
            numEdges = graph.EdgeCount();
            if (numEdges != 5)
            {
            ...
            onProgress("Testing RemoveVertex() with existing edge...");
            graph.RemoveVertex("Galder");
            numVertices = graph.VertexCount();
            if (numVertices != 3)
            {
                onError($"Error. VertexCount() returned {numVertices} instead of 3");
                return false;
            }
            numEdges = graph.EdgeCount();
            if (numEdges != 3)
            {
                onError($"Error. RemoveVertex() failed to remove edges connected to the element removed");
                return false;
            }
            onProgress("Ok.");

            onProgress("Testing RemoveVertex() with non-existing edge...");
            numEdges = graph.EdgeCount();
            if (numEdges != 3)
            {
                onError($"Error. EdgeCount() returned {numEdges} instead of 3");
                return false;
            }
        ...
        public static bool TestDirectedGraphs(Action<string> onProgress, Action<string> onError)
        {
            ...
            if (numEdges != 7)
            {
                onError($"Error. EdgeCount() returned {numEdges} instead of 7");
                return false;
            }
            ...
            {
                onError($"Error. GetEdgeWeight(\"Maider\",\"Itxaso\") returned {graph.GetEdgeWeight("Maider", "Itxaso")} instead of 5.2");
                return false;
            }
            ...
            if (numEdges != 6)
            {
                onError($"Error. After removing an edge (\"Alaitz\",\"Galder\") EdgeCount() returned {numEdges} instead of 6");
                return false;
            }
            onProgress("Ok.");
            ...
            if (numEdges != 4)
            {
                onError($"Error. RemoveVertex() failed to remove edges from/to removed element");
                return false;
            }
            onProgress("Ok.");
            ...
            if (numEdges != 4)
            {
                onError($"Error. EdgeCount() returned {numEdges} instead of 4");
                return false;
            }
        ...
        public static bool TestDistanceCalculator(Action<string> onProgress, Action<string> onError)
        {
            ...
            for (int i = 0; i < numSamples; i++)
            {
                ...
                {
                    onError($"Calculated distance between \"Vitoria-Gasteiz\" and \"Donostia-SS\" is {distances.Get("Donostia-SS")} instead of 108.7");
                    return false;
                }
                if (Math.Abs(distances.Get("Lekeitio") - 136.6) > 0.1)
                {
                    onError($"Calculated distance between \"Vitoria-Gasteiz\" and \"Lekeitio\" is {distances.Get("Lekeitio")} instead of 136.6");
                    return false;
                }
                if (Math.Abs(distances.Get("Santander") - 165.1) > 0.1)
                {
                    onError($"Calculated distance between \"Vitoria-Gasteiz\" and \"Santander\" is {distances.Get("Lekeitio")} instead of 165.1");
                    return false;
                }
                ...
                {
                    onError($"Calculated distance between \"Vitoria-Gasteiz\" and \"Lekeitio\" is {distances.Get("Lekeitio")} instead of 116");
                    return false;
                }
                if (Math.Abs(distances.Get("Santander") - 212.9) > 0.1)
                {
                    onError($"Calculated distance between \"Donostia-SS\" and \"Santander\" is {distances.Get("Santander")} instead of 212.9");
                    return false;
                }
            }
